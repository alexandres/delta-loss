...
def make_shrink_dataloader(dataloader, sz):
    if sz == 0:
        return dataloader
    return ShrinkDataloader(dataloader, sz)

class ShrinkDataloader(torch.utils.data.DataLoader):
    def __init__(self, dataloader, sz):
...
def generate_run_id():
    hashstr = f"{adjectives[random.randint(0, len(adjectives))]}-{names[random.randint(0, len(names))]}" # type: ignore
    return hashstr
...
def get_optimizer(optim_args, model_args, net, state_dict):
    ...
    if optim_args.lr_throttle:
        ...
        base_params = [p for _, p in net.named_parameters()]
        recur_params = []
        iters = 1
        all_params = [{"params": base_params}]

    if optimizer_name == "sgd":
        optimizer = SGD(all_params, lr=lr, weight_decay=optim_args.weight_decay, momentum=0.9)
    elif optimizer_name == "adam":
        optimizer = Adam(all_params, lr=lr, weight_decay=optim_args.weight_decay)
    elif optimizer_name == "adamw":
        optimizer = AdamW(all_params, lr=lr, weight_decay=optim_args.weight_decay)
    else:
...
def load_model_from_checkpoint(problem, model_args, device):
    ...
    if hasattr(model_args, "recall_relu"):
        recall_relu = model_args.recall_relu
    else:
        recall_relu = False
    epoch = 0
    state_dict = None
    ...
    net = get_model(model, width, in_channels=in_channels, max_iters=max_iters, recall_relu=recall_relu)
    net = net.to(device)
    if device == "cuda":
    ...
    if model_path is not None:
        ...
        logging.info(f"{state_dict['epoch']=}")
        epoch = state_dict["epoch"] + 1

    return net, epoch, state_dict
