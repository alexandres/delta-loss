...
def main(cfg: DictConfig):
    ...
    loaders["train"] = dt.utils.make_shrink_dataloader(loaders["train"], cfg.problem.train_batches)
    loaders["val"] = dt.utils.make_shrink_dataloader(loaders["val"], cfg.problem.val_batches)
    loaders["test"] = dt.utils.make_shrink_dataloader(loaders["test"], cfg.problem.test_batches)

    net, start_epoch, state_dict = dt.utils.load_model_from_checkpoint(cfg.problem.name,
                                                                                 cfg.problem.model,
                                                                                 device)
    ...
    acc, val_acc, test_acc = 0.0, 0.0, 0.0

    val_loader = loaders["val"]

    if start_epoch > 0:
        highest_val_acc_so_far = dt.test(net, [val_loader], cfg.problem.hyp.test_mode, [cfg.problem.model.max_iters],
                          cfg.problem.name, device)[0][cfg.problem.model.max_iters]

    for epoch in range(start_epoch, cfg.problem.hyp.epochs):
        train_loader = loaders["train"]
        loss, acc = dt.train(net, train_loader, cfg.problem.hyp.train_mode, train_setup, device, writer, epoch)
        val_acc = dt.test(net, [val_loader], cfg.problem.hyp.test_mode, [cfg.problem.model.max_iters],
                          cfg.problem.name, device)[0][cfg.problem.model.max_iters]
        ...
        if (epoch + 1) % cfg.problem.hyp.val_period == 0 or epoch + 1 == cfg.problem.hyp.epochs:
            ...
            max_iters = cfg.problem.model.max_iters
            lg.write_to_tb([val_acc, test_acc[tb_last], test_acc[max_iters], max(test_acc.values())],
                           ["val_acc", "test_acc", "test_acc_max_iters", "test_acc_max"],
                           epoch,
                           writer)
        ...
        save_now = save_now and os.environ.get("SKIPSAVE", "0") != "1"
        if save_now:
            state = {"net": net.state_dict(), "epoch": epoch, "optimizer": optimizer.state_dict(), 
                     "lr_scheduler": lr_scheduler.state_dict(), "warmup_scheduler": warmup_scheduler.state_dict() }
...
if __name__ == "__main__":
    # if +run_id={run_id} not in argv, then gen and add it
