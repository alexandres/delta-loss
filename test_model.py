...
def main(cfg: DictConfig):
    ...
    train_cfg_path = os.path.join(cfg.problem.model.model_path, ".hydra/config.yaml")
    log.info(f"{train_cfg_path=}")

    training_args = OmegaConf.load(train_cfg_path)
    log.info(f"{training_args=}")
    ...
    for k1, k2 in cfg_keys_to_load:
        try:
            cfg["problem"][k1][k2] = training_args["problem"][k1][k2] # type: ignore
        except KeyError as e:
            log.info(e)
            log.info(f"couldnt find {k1=} {k2=} in cfg_keys_to_load")
    ...
    cfg.problem.model.model_path = os.path.join(cfg.problem.model.model_path, f"model_{cfg.problem.model.which}.pth")
